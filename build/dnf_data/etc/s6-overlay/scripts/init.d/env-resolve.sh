...
strip_quotes \
    ...
    GATE_REGISTRATION_OPEN \
    GAME_SERVER_IP PUBKEY_OVERWRITE
...
if ! mysql_external_configured; then
...
# 服务端默认监听端口
# init-data 将其替换进 cfg 的 __标记__, 端口探针直接读取这些配置
for _port_def in \
...
# secagent shm 配置
if [ -z "$SECAGENT_CHANNEL_NUM" ]; then
    open_channel_count=$(count_open_channels "$OPEN_CHANNEL")
    if [ "${open_channel_count:-0}" -gt 0 ]; then
        SECAGENT_CHANNEL_NUM=$open_channel_count
    else
        SECAGENT_CHANNEL_NUM=12
    fi
fi
echo "secagent channel num is $SECAGENT_CHANNEL_NUM"
...
# 扩展的数据库用户
DNF_DB_USER_EXTENDED="${DNF_DB_USER_EXTENDED:-}"
DNF_DB_USER_EXTENDED="${DNF_DB_USER_EXTENDED//\'/}"
DNF_DB_USER_EXTENDED="${DNF_DB_USER_EXTENDED//\"/}"
...
for v in MAIN_BRIDGE_IP SERVER_GROUP SERVER_GROUP_NAME SERVER_GROUP_DB \
    ...
    DNF_DB_ROOT_PASSWORD DNF_DB_GAME_PASSWORD DEC_GAME_PWD DNF_DB_USER_EXTENDED \
    MALLOC_CONF MALLOC_CONF_32 MALLOC_CONF_64 CLIENT_POOL_SIZE \
    AUTO_PUBLIC_IP PUBLIC_IP \
    DDNS_ENABLE DDNS_DOMAIN DDNS_INTERVAL \
    NB_SETUP_KEY NB_MANAGEMENT_URL \
    ...
    GATE_TLS_BIND_ADDRESS GATE_TLS_ONLY GATE_TLS_CERT_PATH GATE_TLS_KEY_PATH \
    GATE_REGISTRATION_OPEN PUBKEY_OVERWRITE; do
    write_env "$v"
done
